...
# Resource group for RFF React Prod
...
# Storage Account for React app static files (Production-grade)
resource "azurerm_storage_account" "app" {
  ...
  blob_properties {
    versioning_enabled = true # Enable versioning for production
  ...
  lifecycle {
    prevent_destroy = true # Protect production storage
  }
}

# Key Vault for secrets management (Production-grade)
resource "azurerm_key_vault" "app" {
  ...
  sku_name                   = "standard"
  soft_delete_retention_days = 90 # Maximum retention for production
  purge_protection_enabled   = true # Prevent purging in production

  network_acls {
    bypass         = "AzureServices"
    default_action = "Deny" # Restrict access in production
  ...
  lifecycle {
    prevent_destroy = true # Protect production secrets
  }
}
